...
ebd <- read_ebd("ebd_US-WI_doveki_relJul-2022.txt", unique = FALSE, rollup = FALSE)
...
sens <- read_ebd("GOODsens_ebd_relJan-2022_US-WI_2015-2019atlaswi.txt", unique = FALSE, rollup = FALSE)
...
atlasers <- read.csv("ebd_users_relJun-2022.txt")
...
details <- file.info(list.files("C:/Users/nicho/Desktop/Non_EBD_Auto_Reports/July2022", 
...
write.delim(ebdplussensitivepluszerocount, file = "22EBDmergedwithSensitiveSpeciesandZeroCountRecordsNEW2.txt", sep="\t")
...
fullebd <- read.delim("ebd_US-WI_201501_201912_relJul-2022.txt", sep="\t", header=TRUE, quote = "", stringsAsFactors = FALSE, na.strings=c(""))
...
sensandzerocount <- read.delim("22EBDmergedwithSensitiveSpeciesandZeroCountRecords.txt", sep="\t", header=TRUE, quote = "", stringsAsFactors = FALSE, na.strings=c(""))
...
#export file
library(pgirmess) 
write.delim(newebd, file = "EBDplussensitivepluszerocountJul2022.txt", sep="\t")
