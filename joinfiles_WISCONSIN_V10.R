...
ebd <- read_ebd("ebd_evegro_201501_201911_relJul-2023.txt", unique = FALSE, rollup = FALSE)
...
sens <- read_ebd("SENSebd_relMar-2023_US-WI_1995-2000.txt", unique = FALSE, rollup = FALSE)
...
details <- file.info(list.files("C:/Users/nicho/Desktop/Non_EBD_Auto_Reports/April2023forWBBAI", 
...
write.delim(ebdplussensitivepluszerocount, file = "TESTplussensitivepluszerocountbetter3.txt", sep="\t")
...
# load the full ebd
#library(auk)
#fullebd <- read_ebd("ebd_US-WI_201501_201912_relJul-2022.txt", unique = FALSE)
fullebd <- read.delim("EBDATLASWIplussensitivepluszerocountMar2023_GOOD.txt", sep="\t", header=TRUE, quote = "", stringsAsFactors = FALSE, na.strings=c(""))


fullebd <- mutate(fullebd, id = case_when(
  id == 30 ~ 40, 
  TRUE   ~ id 
))
df

fullebd$BREEDING.CODE[fullebd$BREEDING.CODE == "NC" & fullebd$GLOBAL.UNIQUE.IDENTIFIER == 'URN:CornellLabOfOrnithology:EBIRD:OBS787895092'] <- "H"

fullebd$BREEDING.CATEGORY[fullebd$BREEDING.CATEGORY == "C1" & fullebd$GLOBAL.UNIQUE.IDENTIFIER == 'URN:CornellLabOfOrnithology:EBIRD:OBS787895092'] <- "C2"
...
# limits dataset to only WI atlas portal records (change this to your atlas if desired)
fullebd  <- fullebd[fullebd$PROJECT.CODE == "EBIRD_ATL_WI", ]
...
# OR LOAD IN THE TABLES IF YOU EXPORTED THEM ALREADY AND MANUALLY REMOVED THE DUMMY EBD
sensandzerocount <- read.delim("Apr23EBDmergedwithSensitiveSpeciesandZeroCountRecordsDateFixed.txt", sep="\t", header=TRUE, quote = "", stringsAsFactors = FALSE, na.strings=c(""))

#make sure NAs are really NA
sensandzerocount[sensandzerocount=="NA"]=NA

library(stringr)
# make column names uppercase again
names(sensandzerocount) <- toupper(names(sensandzerocount))
colnames(sensandzerocount) <- str_replace_all(colnames(sensandzerocount), "[:punct:]", ".")

# append table onto bottom of ebd
newebd <- rbind(fullebd, sensandzerocount)

#export file
library(pgirmess) 
write.delim(fullebd, file = "EBDATLASWIplussensitivepluszerocountMar2023_GOOD2.txt", sep="\t")
